...
/**
 * Performs fast modular exponentiation using exponentiation by squaring and is
 * suitable for operations on large numbers typically seen in cryptography.
 *
 * The exponentiation `**` operator is not suitable for extremely large numbers
 * such as those used with cryptography. The naive approach for exponentiation
 * followed by a modulus results in extremely large numbers that will overflow
 * large number libraries. The use of a modular exponentiation method is used to
 * iteratively exponentiate and pass through the modulus to keep the operation
 * memory efficient.
 *
 * This version of modular exponentation is implemented using the binary
 * right-to-left method as defined in Applied Cryptograph by Bruce Schneier and
 * found on [wikipedia](https://en.wikipedia.org/wiki/Modular_exponentiation#Pseudocode).
 *
 * @param base base
 * @param exp exponent
