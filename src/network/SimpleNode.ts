...
export class SimpleNode extends EventEmitter {
  ...
  public send(msg: INetworkMessage) {
    ...
    if (this.logging) {
      console.log("node: sending", envelope.toString().substring(0, 255));
    }
  ...
  private _onReadable() {
    while (true) {
      ...
      if (this.logging) {
        console.log("node: received", env.toString().substring(0, 128));
      }
      ...
      switch (env.command) {
        ...
        case "headers":
          ...
          break;
        case "merkleblock":
          const merkleblock = MerkleBlock.parse(env.payloadStream);
          this.emit("merkleblock", merkleblock);
          break;
        case "tx":
          const tx = Tx.parse(env.payloadStream, this.testnet);
          this.emit("tx", tx);
          break;
      }
    }
